...
def all_teachers_join
  ...
  execute(<<-SQL)
  select name,dept from teachers left outer join (select id,name as dept from depts) as renamed on teachers.dept_id=renamed.id 
...
def all_depts_join
  ...
  execute(<<-SQL)
select teachers.name,depts.name from depts left outer join teachers on depts.id = teachers.dept_id
  SQL
end
...
def teachers_and_depts
  ...
  # department.
  execute(<<-SQL)
  select teachers.name,coalesce(depts.name,'None') from teachers left join depts on teachers.dept_id=depts.id  
  SQL
end
...
def dept_staff_counts
  ...
  execute(<<-SQL)
  select depts.name,count(teachers.name) from depts left outer join teachers on depts.id = teachers.dept_id group by depts.name
  SQL
end
